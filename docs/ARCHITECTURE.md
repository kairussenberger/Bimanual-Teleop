...
| 8b | Hand min-separation | `safety/separation.py` via `engine.py` | the two hand CAPSULES (or a parked arm's) kept ≥ `safety.hand_min_separation` apart in 3D — clapped hands meet at contact distance, never interpenetrate. Near-parallel capsules (a full clasp) resolve along the WRIST line, not the closest-point gradient, so the hands stop face-to-face instead of shearing past each other ("crossing") | `tests/test_guardrails.py` capsule/clasp/poke + `tests/test_neutral_calib.py` separation tests |
